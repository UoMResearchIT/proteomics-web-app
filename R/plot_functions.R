...
generate_heatmap_colors <- function(data){
  x <- as.matrix(select_if(data, is.numeric)) # modified here
  ...
  col_fun <- colorRamp2(breaks = c(x.trim[x.min], 0, x.trim[x.max]), # modified here
...
# ### added this just to test the generate_heatmap_color function above
heatmap_colors <- generate_heatmap_colors(protein_heatmap)
...
make_heatmap <- function(data, heatmap_colors){
  data_m <- as.matrix(select_if(data, is.numeric)) # modified here
  set.seed(3)
